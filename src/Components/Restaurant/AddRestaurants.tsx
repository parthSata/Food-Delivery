...
function AddRestaurants() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isFieldEmpty(restaurant.restaurantName)) newErrors.restaurantName = "Restaurant Name is required";
        if (isFieldEmpty(restaurant.address)) newErrors.address = "Address is required";
        if (isFieldEmpty(restaurant.latitude)) newErrors.latitude = "Latitude is required";
        if (isFieldEmpty(restaurant.longitude)) newErrors.longitude = "Longitude is required";
        if (isFieldEmpty(restaurant.mobilenumber)) newErrors.mobilenumber = "Mobile Number is required";
        if (isFieldEmpty(restaurant.ratings)) newErrors.ratings = "Ratings is required";
        if (isFieldEmpty(restaurant.description)) newErrors.description = "Discription is required";
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/restaurants/${updateId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                    ...
                    <div className="flex  flex-wrap">
                        <div className="flex -order-1 justify-center flex-wrap sm:flex-row md:flex-col xl:flex-row w-auto flex-row mb-10 font-semibold" style={{ fontFamily: "Bai Jamjuree" }}>
                            {[0].map((index) => (
                                <div
                                    key={index}
                                    className={`border-dotted rounded-[15px] border-4 h-[120px] m-6 flex-col gap-2 text-md w-[150px] flex justify-center items-center ${restaurantImages[index] ? 'border-[#DF201F]' : 'border-[#161A1D]'}`}
                                >
                                    {restaurantImages[index] ? (
                                        ...
                                        >
                                            <img src={restaurantImages[index]} alt={`Preview ${index}`} className="h-auto w-auto object-cover" />
                                            <button
                                                type="button"
                    ...
                    <div className="flex items-center w-full order-1" style={{ fontFamily: "Bai Jamjuree" }}>
                        <div className="flex justify-center  font-semibold flex-col text-md items-center m-4 h-[420px] w-[480px]" style={{ boxShadow: "2px 2px 20px 2px #FFE9D066" }}>
                            <div className="border-dotted bg-[#F5F5F5] rounded-[15px] border-4 h-[380px] flex-col gap-2 text-md w-full flex justify-center items-center border-[border: 2px solid #161A1D]">
                                {previewImage ? (
                                    <img src={previewImage} alt="Preview" className="h-[250px] w-full object-cover" />
                                ) : (
