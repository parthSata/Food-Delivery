...
import { signInWithEmailAndPassword, sendEmailVerification, sendSignInLinkToEmail, getAuth, signInWithEmailLink } from "firebase/auth";
  ...
  const sendSignInLink = () => {
    ...
    const handleLogin = async (e?: React.FormEvent<HTMLFormElement>) => {
      ...
      if (Object.keys(newErrors).length === 0 && isValidEmail) {
        try {
          const emailKey = email.replace(/\./g, '_'); // Replace '.' with '_' for Firebase path compatibility
          const userDataRef = ref(db, `users/${emailKey}`);
